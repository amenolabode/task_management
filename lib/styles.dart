import 'package:flutter/material.dart';

const TextStyle header = TextStyle(
    fontFamily: "Poppins",
    fontSize: 32,
    fontWeight: FontWeight.w600,
    color: Colors.white);

const TextStyle cardHeader = TextStyle(
    fontFamily: "Poppins",
    fontSize: 24,
    fontWeight: FontWeight.w600,
    color: Colors.white);

const TextStyle sectionHeader = TextStyle(
    fontFamily: "Poppins",
    fontSize: 20,
    fontWeight: FontWeight.w600,
    color: Colors.white);

const TextStyle subTasks = TextStyle(
    fontFamily: "Poppins",
    fontSize: 16,
    fontWeight: FontWeight.w400,
    color: Colors.white);

const TextStyle bodyTextStyle = TextStyle(
    fontFamily: "Poppins",
    fontSize: 16,
    fontWeight: FontWeight.w400,
    color: Colors.white);

const TextStyle bodyBoldStyle = TextStyle(
    fontFamily: "Poppins",
    fontSize: 16,
    fontWeight: FontWeight.w600,
    color: Colors.white);

const TextStyle darkBoldStyle = TextStyle(
    fontFamily: "Poppins",
    fontSize: 16,
    fontWeight: FontWeight.w600,
    color: backgroundColor);

const TextStyle darkBody2Style = TextStyle(
    fontFamily: "Poppins",
    fontSize: 14,
    fontWeight: FontWeight.w400,
    color: backgroundColor);

const TextStyle bodyTextStyleGrey = TextStyle(
    fontFamily: "Poppins",
    fontSize: 16,
    fontWeight: FontWeight.w400,
    height: 1.4,
    color: Colors.white60);

const Color primaryColor = Color(0xff7D0FEC);
const Color fadedPrimaryColor = Color(0xffB46EF4);
const Color cardColor = Color(0xff272B34);
const Color articleColor = Color(0xff75D9FF);
const Color backgroundColor = Color(0xff1B1E24);
const Color yellowColor = Color(0xffFDD064);
const Color redColor = Color(0xffFF8183);
